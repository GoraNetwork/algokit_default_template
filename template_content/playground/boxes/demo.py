...
def main() -> None:
    ...
    app_client.call(application.add_member, addr=member.address)
    ...
    result = app_client.call(application.read_balance, addr=member.address)
    print(result.return_value)

    # Set the balance and read it back out
    app_client.call(application.set_balance, addr=member.address, amount=100)
    result = app_client.call(application.read_balance, addr=member.address)
    print(result.return_value)
    ...
    box_name = "mybox"
    app_client.call(
        application.fill_box, box_name=box_name, box_data=["hello", "world"]
    )
    # read it back out
